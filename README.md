...
- _Это структура данных, являющаяся В-деревом, каждый узел которого имеет либо два потомка и_ _одно поле, либо три потомка и два поля. 2-3 деревья сбалансированы(все листовые вершины_ _находятся на одной высоте от корня дерева)._
- _Где и как она используется (приложения)?_
- _Операции: поиск ключа, добавление ключа, удаление ключа, слияние, разбиение ключа._ 
- _Теоретическая сложность операций: поиск за `O(log(n))`, вставка за `O(log(n))` удаление за_ _`O(log(n))`._
_
